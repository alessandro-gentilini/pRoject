%************************************************
\section{The \pkg{data.table} package}\label{sec: datatable}
%************************************************
\samp{install.packages(`data.table')}\\
\samp{library(`data.table')}
\bigskip 

A \samp{data.table} is a \df plus
...
A \samp{data.table} does not have row numbers
...
\subsection{Subsetting a data table upon constraints}
The general syntax form for a data table is 
\samp{dt[i,j, by = k]}, meaning to subset the rows using 
\samp{i}, then apply \samp{j} as a function grouped by
\samp{k}. The syntax is totally equivalent to the 
standard \samp{SQL}, with \samp{i,j} replacing 
\samp{where} and \samp{select} clauses, respectively.
...
\samp{iris[, c(`Petal.Width',`Petal.Length') := NULL]}.
\bigskip
...
\end{example}
The \samp{not in} operator in \samp{R} is obtained
...
The \samp{.I} operator shows the row number in 
...
3:  virginica 106
\end{example}
The variable \samp{V1} contains the actual row number 
...
3:  virginica 50
\end{example}
The \samp{.SD} operator creates a data table whose 
...
The \samp{[} operator can be used in sequence to allow 
...
sampling we will make use of the \samp{quarks} data table as 
...
10:   F    down -1/2
\end{example}
The function \samp{unique(dt, by = c(first, second))}
...
variable, the syntax
\samp{merge(first, second, by = c(`var1', `var2'))}
...
syntax \samp{first[second, nomatch=0]} once we set the 
...
6:   F  bottom  1/2   1/2
\end{example}
gives the same results, as also \samp{second[first, nomatch=0]}.
The condition \samp{nomatch=0} ensure the inner join as 
...
\samp{merge(first, second, by = c("flavour", "S\_z"), 
all = TRUE)}

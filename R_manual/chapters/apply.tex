%************************************************
\section{Vectorised operations on data frames}\label{sec: apply}
%************************************************
Given a data frame, the functions of the family
\samp{apply} allow to perform vectorised operations
...
\samp{apply} function, where rows and columns can 
...
\samp{apply(df, margin, FUN = <function> )}, with \samp{margin} being
$1,2,\ldots$ and \samp{<function>} being any function.
\begin{example}
...
specific operators exist as \samp{colSums} and \samp{colMeans},
...
\subsection{lapply and sapply}
\samp{lapply} applies a function to each element of a list
and returns a list back. Equivalently, \samp{sapply} does
...
\subsection{Vectorised assignments}
Vectorised assignments in \samp{R} commute with 
functions, namely the operator \samp{c} is such
that \samp{c(f) = f(c)}.
\begin{example}
...
\end{example}
Likewise, \samp{ifelse} evaluates a given 
...
\end{example}
The function \samp{cumsum} returns the cumulative 
...
values, the operator \samp{Reduce}

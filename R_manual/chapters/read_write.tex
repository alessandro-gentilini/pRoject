%************************************************
\section{Writing and reading data}\label{sec: read_write}
%************************************************
...
\end{example}
The option \samp{na.strings = c("NA", "-")} decides
which lines must be interpreted as \samp{NA}. 
Likewise \samp{fill = TRUE} allows to skip and 
continue whenever inconsistencies in the data are
present: on the other hand \samp{fill = FALSE} 
...
\samp{strip.white = TRUE}.
\bigskip

The \samp{data.table} package makes use of 
\samp{fread} to read data file in, this being
